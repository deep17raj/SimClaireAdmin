...
export default function AdminPlanControlPage() {
  ...
  useEffect(() => {
    fetchPlans();
    // Reset filters when changing country to avoid empty screens
    setPlanSearchQuery("");
    setFilterStatus("all");
  ...
  const handleDelete = async (planID) => {
    ...
    try {
      await axios.delete(`${API_BASE}/admin/plan-control/delete`, {
        data: { plan_id: planID, country_code: countryCode },
        headers: { Authorization: `Bearer ${adminToken}` }
      });
      fetchPlans();
    } catch (err) {
              ...
              <table className="w-full text-left text-sm text-gray-600 min-w-[1100px]">
                <thead className="bg-slate-50 border-b border-gray-200 text-slate-600 font-bold uppercase text-xs tracking-wider">
                  <tr>
                    ...
                    <th className="px-4 py-4 bg-orange-50 border-l border-orange-100 text-center">Multiplier</th>
                    <th className="px-4 py-4 bg-blue-50 border-l border-blue-100 text-right">Final Price</th>
                    <th className="px-4 py-4 bg-emerald-50 border-l border-emerald-100 text-right">Profit Margin</th>
                      ...
                      <React.Fragment key={plan.productID}>
                        ...
                        <tr className={`hover:bg-slate-50 transition-colors ${isExpanded ? 'bg-slate-50' : ''}`}>
                         ...
                         <td className="px-4 py-4 font-medium text-gray-700">
                          ...
                          <td className="px-4 py-4 text-center bg-orange-50/30 border-l border-orange-100">
                            {isEditing ? (
                              <input 
                                type="number"
                                step="0.1"
                                ...
                                className="w-20 px-2 py-1.5 border border-orange-300 rounded-md text-center text-gray-900 font-bold focus:ring-2 focus:ring-orange-500 outline-none shadow-sm"
                              />
                            ) : (
                          ...
                          <td className="px-4 py-4 text-center">
                            {isEditing ? (
                              ...
                              <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold inline-block border ${
